...
public void OnPluginStart()
{
	...
	AddCommandListener(CmdMapChange, "sm_map");
...
stock bool L4D2_ChangeLevel(const char[] sMapName)
{
	...
	if(sMapName[0] == '\0' || FindMap(sMapName, temp, sizeof(temp)) == FindMap_NotFound)
	{
		LogError("Unable to change to that map \"%s\"", sMapName);
		return false;
	}
	...
	if(hInfoMapChange == null)
	{
		...
		if(hGamedata != null) 
		{
			...
			if(PrepSDKCall_SetFromConf(hGamedata, SDKConf_Signature, "InfoChangelevel::ChangeLevelNow"))
			{
				...
				if(hInfoMapChange == null)
					LogError("Unable to prep SDKCall 'InfoChangelevel::ChangeLevelNow'");
			}
			else
			{
				LogError("Error finding the 'InfoChangelevel::ChangeLevelNow' signature.");
			}
		}
		else
		{
			LogError("Failed to load \"l4d2_changelevel.txt\" gamedata.");
		}
		delete hGamedata;
		if(hInfoMapChange == null)
			return false;
